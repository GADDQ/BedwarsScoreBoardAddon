...
public class Heal implements Upgrade {
	...
	public Heal(Game game, RunningTeam team, int level) {
		this.game = game;
		this.team = team;
		this.level = level;
		i = 0;
	...
	public void runUpgrade() {
		...
		for (Player player : team.getConnectedPlayers()) {
			if (!BedwarsUtil.isSpectator(game, player) && player.getGameMode() != GameMode.SPECTATOR) {
				if (team.getTargetBlock().distance(player.getLocation()) <= Config.teamshop_upgrade_heal_trigger_range) {
		...
		if (i >= 6) {
			i = 0;
			Location location = team.getTargetBlock().clone().add(0.5, 1, 0.5);
			for (Player player : team.getConnectedPlayers()) {
				for (int i = 0; i < 10; i++) {

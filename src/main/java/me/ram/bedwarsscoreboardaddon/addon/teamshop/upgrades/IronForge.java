...
public class IronForge implements Upgrade, Listener {
	...
	public IronForge(Game game, RunningTeam team, int level) {
		this.game = game;
		this.team = team;
		this.level = level;
	...
	private void dropItem(Location loc, Material type) {
		...
		//Boolean inchest = block.getType().equals(Material.CHEST) && BedwarsAPI.getInstance().getBooleanConfig("spawn-resources-in-chest", true);
	...
	private List<Player> getNearbyPlayers(Location location, ItemStack item) {
		...
		for (Entity entity : location.getWorld().getNearbyEntities(location, Config.spread_resource_range, 0.5, Config.spread_resource_range)) {
			if (entity instanceof Player) {
				...
				if (game.isPlayerInAnyTeam(player) && !BedwarsUtil.isSpectator(game, player) && isCanContain(player, item)) {
					players.add(player);
				}
			}
		}

...
public class InvisibilityPlayer {
	...
	public void hidePlayer(Player player) {
		...
		arena.addGameTask(new BukkitRunnable() {
			@Override
			public void run() {
				if (!player.isOnline() || !player.hasPotionEffect(PotionEffectType.INVISIBILITY) || !players.contains(player.getUniqueId()) || !game.getConnectedPlayers().contains(player) || game.isSpectator(player)) {
					cancel();
					task.cancel();
					players.remove(player.getUniqueId());
					hplayers.remove(player.getUniqueId());
	...
	private void hideArmor(Player player) {
		if (BedwarsUtil.getCurrentVersion().startsWith("v1_8")) {
			try {
				...
				List<Player> players = game.getTeamOfPlayer(player).getConnectedPlayers();

...
public class ResourceUpgrade {
	...
	public ResourceUpgrade(Arena arena) {
		...
		for (ItemSpawner spawner : game.getItemSpawners()) {
			ItemStack itemStack = spawner.getItemSpawnerType().getStack();
			levels.put(itemStack.getType(), "I");
			interval.put(itemStack.getType(), (int) (spawner.getItemSpawnerType().getInterval() / 50));

			Location sloc = spawner.getLocation();
			ItemStack itemStack2 = spawner.getItemSpawnerType().getStack();
				arena.addGameTask(new BukkitRunnable() {
					...
					@Override
					public void run() {
						spawn_time.put(itemStack2.getType(), ((int) (i / 20) + 1));
						if (i <= 0) {
							i = interval.get(itemStack2.getType());
							int es = 0;
							for (Entity entity : loc.getWorld().getNearbyEntities(loc, 2, 2, 2)) {
								if (entity instanceof Item) {
									Item item = (Item) entity;
									if (item.getItemStack().getType().equals(itemStack2.getType())) {
							...
							if (Config.resourcelimit_enabled) {
								for (String[] rl : Config.resourcelimit_limit) {
									if (rl[0].equals(itemStack2.getType().name()) && es >= Integer.valueOf(rl[1])) {
										drop = false;
									}
								}
							}
							...
							if (drop /*|| inchest*/) {
								BedwarsResourceSpawnEvent event = new BedwarsResourceSpawnEvent(game, spawner, itemStack2.clone());
								Bukkit.getPluginManager().callEvent(event);
								if (!event.isCancelled()) {
									/*if (inchest && spawner.canContainItem(((Chest) block.getState()).getInventory(), itemStack2)) {
										((Chest) block.getState()).getInventory().addItem(itemStack2.clone());
									} else*/ if (drop) {
										ConfigurationSection config = Main.getInstance().getConfig().getConfigurationSection("holographic.resource.resources");
										String res_name = getResourceName(itemStack2.getTypeId());
										double i = res_name == null || !config.getBoolean(res_name + ".drop", false) ? 0 : config.getDouble(res_name + ".height", 0.0);
										i = i > 0 ? i : 0.325;
										Item item = loc.getWorld().dropItem(loc.clone().add(0, i, 0), itemStack2);
		...
		for (String rs : Main.getInstance().getConfig().getConfigurationSection("resourceupgrade").getKeys(false)) {
			arena.addGameTask(new BukkitRunnable() {
				...
				public void run() {
					...
					int remtime = EventListener.countdownMap.get(game) - gametime;
					String formatremtime = remtime / 60 + ":" + ((remtime % 60 < 10) ? ("0" + remtime % 60) : (remtime % 60));
					upg_time.put(rs, formatremtime);
					if (EventListener.countdownMap.get(game) <= gametime) {
						isExecuted = true;

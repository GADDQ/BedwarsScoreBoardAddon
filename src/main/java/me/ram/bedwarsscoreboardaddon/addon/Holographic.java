...
public class Holographic {
			...
			new BukkitRunnable() {
				...
				public void run() {
					for (RunningTeam team : game.getRunningTeams()) {
						Location location = team.getTargetBlock().clone().add(0.5, 0, 0.5);
	...
	public void onTargetBlockDestroyed(BedwarsTargetBlockDestroyedEvent e) {
		if (Config.holographic_bed_title_bed_destroyed_enabled) {
			...
			Location loc = team.getTargetBlock().clone().add(0, -1, 0);
			...
			HolographicAPI holo = new HolographicAPI(loc, Config.holographic_bedtitle_bed_destroyed_title.replace("{player}", ColorUtil.translateToChatColor(game.getTeamOfPlayer(e.getPlayer()).getColor()) + e.getPlayer().getName()));
			btitles.add(holo);
	...
	public void onPlayerJoin(Player player) {
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				if (game.getStatus() == GameStatus.RUNNING && player.isOnline() && game.getConnectedPlayers().contains(player)) {
					for (HolographicAPI holo : ablocks) {
						holo.display(player);
					}
					for (HolographicAPI holo : atitles) {
	...
	public void onArmorStandManipulate(PlayerArmorStandManipulateEvent e) {
		...
		Game getGame = BedwarsAPI.getInstance().getGameOfPlayer(player);
		if (getGame == null) {
			return;
		}
		if (!getGame.getConnectedPlayers().contains(player)) {
			return;
		}
		if (getGame.getStatus() != GameStatus.WAITING && getGame.getStatus() == GameStatus.RUNNING) {
			e.setCancelled(true);
		}
	}

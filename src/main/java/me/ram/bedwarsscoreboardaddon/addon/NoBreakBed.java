...
public class NoBreakBed {
	...
	public NoBreakBed(Arena arena) {
		...
		arena.addGameTask(new BukkitRunnable() {
			@Override
			public void run() {
				int time = EventListener.countdownMap.get(game) - Config.nobreakbed_gametime;
				String ftime = time / 60 + ":" + ((time % 60 < 10) ? ("0" + time % 60) : (time % 60));
				formattime = ftime;
				if (EventListener.countdownMap.get(game) <= Config.nobreakbed_gametime) {
					bre = true;
					if (Config.nobreakbed_enabled) {
	...
	private void registerPacketListener() {
		packetListener = new PacketAdapter(Main.getInstance(), ListenerPriority.HIGHEST, new PacketType[] { PacketType.Play.Client.BLOCK_DIG }) {
			public void onPacketReceiving(PacketEvent e) {
				...
				if (BedwarsUtil.isSpectator(game, player) || game.getStatus() != GameStatus.RUNNING) {
					return;
				}

...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				if (Bukkit.getPluginManager().getPlugin("BedWars") == null || Bukkit.getPluginManager().getPlugin("Citizens") == null || Bukkit.getPluginManager().getPlugin("ProtocolLib") == null || (Bukkit.getPluginManager().isPluginEnabled("BedWars") && Bukkit.getPluginManager().isPluginEnabled("Citizens") && Bukkit.getPluginManager().isPluginEnabled("ProtocolLib"))) {
					cancel();
					printMessage("§f===========================================================");
					printMessage("§7 ");
					...
					printMessage("§7 ");
					printMessage("§f  " + getLocaleConfig().getLanguage("author") + ": §aEarth_Studio, TheRamU");
					printMessage("§7 ");
					printMessage("§f  " + getLocaleConfig().getLanguage("website") + ": §ehttps://github.com/GADDQ/BedwarsScoreBoardAddon");
					printMessage("§7 ");
					printMessage("§f===========================================================");
					init();
				}
	...
	private void init() {
		...
		if (Bukkit.getPluginManager().getPlugin("BedWars") != null) {
			/*
			if (!Bukkit.getPluginManager().getPlugin("BedWars").getDescription().getVersion().equals("1.3.6")) {
		...
		try {
			...
			metrics.addCustomChart(new Metrics.SimplePie("pluginPrefix", new Callable<String>() {
				...
				public String call() throws Exception {
					return ChatColor.GRAY + "[" + ChatColor.AQUA + "BedWars" + ChatColor.GRAY + "]";

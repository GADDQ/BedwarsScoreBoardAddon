	...
	public static void setLobbyScoreboard(Player player, String title, List<String> lines, Game game) {
		...
		try {
			...
			for (RunningTeam t : game.getRunningTeams()) {
				...
				team.setPrefix(ColorUtil.translateToChatColor(t.getColor()).toString());
		...
		try {
			...
			for (RunningTeam team : game.getRunningTeams()) {
				...
				if (!Config.playertag_prefix.equals("")) {
					score_team.setPrefix(Config.playertag_prefix.replace("{color}", ColorUtil.translateToChatColor(team.getColor()) + "").replace("{color_initials}", ColorUtil.translateToChatColor(team.getColor()).name().substring(0, 1)).replace("{color_name}", upperInitials(ColorUtil.translateToChatColor(team.getColor()).name())).replace("{team_initials}", team.getName().substring(0, 1)).replace("{team}", team.getName()));
				}
				if (!Config.playertag_suffix.equals("")) {
					score_team.setSuffix(Config.playertag_suffix.replace("{color}", ColorUtil.translateToChatColor(team.getColor()) + "").replace("{color_initials}", ColorUtil.translateToChatColor(team.getColor()).name().substring(0, 1)).replace("{color_name}", upperInitials(ColorUtil.translateToChatColor(team.getColor()).name())).replace("{team_initials}", team.getName().substring(0, 1)).replace("{team}", team.getName()));
				}
				score_team.setAllowFriendlyFire(false);
				for (Player pl : team.getConnectedPlayers()) {
					if (!score_team.hasPlayer((OfflinePlayer) pl)) {
						if (!players.contains(pl.getUniqueId()) || (player_team != null && player_team.getConnectedPlayers().contains(pl))) {
					...
					if (!team.getName().equals(player_team.getName())) {
						for (Player pl : team.getConnectedPlayers()) {
							...
							for (org.bukkit.scoreboard.Team score_team : scoreboard2.getTeams()) {
								if (player_team.getConnectedPlayers().contains(player)) {

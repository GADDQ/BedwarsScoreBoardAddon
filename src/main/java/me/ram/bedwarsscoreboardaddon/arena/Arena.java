...
public class Arena {
		...
		addGameTask(new BukkitRunnable() {
			...
			public void run() {
				if (!game.getStatus().equals(GameStatus.RUNNING)) {
					onOver(new BedwarsGameEndingEvent(game, null));
					onEnd();
				}
			}
	...
	public void onPlayerKilled(BedwarsPlayerKilledEvent e) {
		...
		if (!game.getConnectedPlayers().contains(player) || !game.getConnectedPlayers().contains(killer) || game.isSpectator(player) || game.isSpectator(killer)) {
			return;
		}
	...
	public void onOver(BedwarsGameEndingEvent e) {
		if (e.getGame().getName().equals(this.game.getName())) {
			isOver = true;
			if (Config.overstats_enabled && e.getWinningTeam() != null) {
				...
				for (Player player : winner.getConnectedPlayers()) {
	...
	public void onPlayerLeave(Player player) {
		...
		if (Config.rejoin_enabled) {
			if (game.getStatus() == GameStatus.RUNNING && !game.isSpectator(player)) {
				...
				if (team != null) {
					if (team.getConnectedPlayers().size() > 1 && !team.isDead()) {
